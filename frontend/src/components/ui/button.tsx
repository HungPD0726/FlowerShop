...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", size = "md", asChild = false, isLoading = false, children, disabled, ...props }, ref) => {
    const baseStyles =
      "inline-flex min-h-11 shrink-0 items-center justify-center whitespace-nowrap rounded-full font-semibold transition-[transform,background-color,color,border-color,box-shadow] duration-500 ease-editorial focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent/20 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]";
    const variants = {
    ...
    return (
      <button ref={ref} className={styles} disabled={disabled || isLoading} aria-busy={isLoading || undefined} {...props}>
        ...
        <span className={cn("inline-flex items-center gap-inherit", isLoading && "opacity-80")}>{children}</span>
      </button>
    );
  }
);
