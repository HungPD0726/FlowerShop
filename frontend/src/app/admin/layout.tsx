    ...
    <AuthGate role="ROLE_ADMIN">
      <div className="flex min-h-dvh bg-canvas text-ink">
        <aside className={`sticky top-0 hidden h-dvh shrink-0 flex-col border-r border-line bg-surface transition-[width] duration-500 ease-editorial lg:flex ${collapsed ? "w-20" : "w-64"}`}>
          <Link href="/admin/dashboard" className="flex h-20 items-center gap-3 border-b border-line px-5"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-[10px] bg-ink font-serif text-xl text-surface">&</span>{!collapsed && <span><span className="block font-serif text-xl font-semibold">Lá & Hoa</span><span className="block text-[10px] font-bold uppercase tracking-[0.14em] text-accent">Quản trị</span></span>}</Link>
          ...
          <div className="mt-auto border-t border-line p-3">
            ...
            <Link href="/" className="flex min-h-11 items-center gap-3 rounded-[10px] px-3 text-sm font-bold text-muted hover:bg-canvas hover:text-ink"><Storefront className="shrink-0" />{!collapsed && "Về cửa hàng"}</Link>
            <button type="button" onClick={signOut} className="flex min-h-11 w-full items-center gap-3 rounded-[10px] px-3 text-sm font-bold text-danger hover:bg-danger/10"><SignOut className="shrink-0" />{!collapsed && "Đăng xuất"}</button>
          </div>
